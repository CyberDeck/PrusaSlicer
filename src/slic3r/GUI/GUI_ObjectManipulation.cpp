...
//     m_og->append_single_option_line(Option(def, "object_name"));
    ...
    m_mirror_bitmap_hidden = ScalableBitmap(parent, "colorchange_add_transparent.png");
	...
	for (const std::string axis : { "x", "y", "z" }) {
        ...
        auto mirror_button = [=](wxWindow* parent) {
            ...
            auto btn = new ScalableButton(parent, wxID_ANY, "colorchange_delete_off.png", wxEmptyString, btn_size, wxDefaultPosition, wxBU_EXACTFIT | wxNO_BORDER | wxTRANSPARENT_WINDOW);
            btn->SetBitmapDisabled(m_mirror_bitmap_off.bmp());
    ...
    wxGetApp().CallAfter([this, new_states]{
        for (int i=0; i<3; ++i) {
            if (new_states[i] != m_mirror_buttons[i].second) {
                const wxBitmap* bmp;
                switch (new_states[i]) {
                    case mbHidden : bmp = &m_mirror_bitmap_hidden.bmp(); break;
                    case mbShown  : bmp = &m_mirror_bitmap_off.bmp(); break;
                    case mbActive : bmp = &m_mirror_bitmap_on.bmp(); break;
                }
